 ...
 $(document).ready(function() {
  ...
  var colArray = $('#metric-table #tablefield-0 td:nth-child(2)').map(function(){
       return $(this).text();
   }).get();
   ...
   var maxVal = parseFloat(colArray[lastVal]);
    $( "#slider-range, #slider-range2" ).slider({
      ...
      slide: function( event, ui ) {
        ...
        var table = $("#metric-table #tablefield-0")[0];
  ...
  var impcolArray = $('#imperial-table #tablefield-0 td:nth-child(2)').map(function(){
       return $(this).text();
   }).get();
   ...
   var impmaxVal = parseFloat(impcolArray[implastVal]);
    $( "#slider-range-imp"  ).slider({
      ...
      values: [ impminVal, impmaxVal ],
        slide: function( event, ui ) {
      var table = $("#imperial-table #tablefield-0")[0];
        // in this function we can define what happens when a user changes the sliders
